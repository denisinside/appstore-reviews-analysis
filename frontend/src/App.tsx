...
import { analyzeScan, createScan, downloadPdfReportUrl, downloadReportUrl, downloadReviewsUrl, getInsights, getIssues, getMetrics, getScan, getScanOptions } from './api/client'
...
function StatusPanel({ scan, onRetry, retrying, retryError }: { scan: Scan; onRetry: () => void; retrying: boolean; retryError?: string }) {
  ...
  const active = scan.analysis_status === 'queued' || scan.analysis_status === 'running'
  const progress = scan.progress
  const percent = Math.max(0, Math.min(100, Math.round(progress?.percent ?? 0)))
  const hasReviewCount = typeof progress?.completed === 'number' && typeof progress?.total === 'number'
  return <div className="mx-auto mt-14 max-w-[740px] panel"><div className="p-8 md:p-12"><p className={`eyebrow ${failed ? '!text-danger' : '!text-orange'}`}>{failed ? 'Analysis failed' : scan.analysis_status === 'not_started' ? 'Analysis ready' : 'Analysis running'}</p><h2 className="mt-4 font-serif text-4xl">{failed ? 'The analysis stopped.' : scan.analysis_status === 'not_started' ? 'Ready to analyze.' : 'Reading the review sample.'}</h2><div className="mt-9 grid grid-cols-2 gap-6 border-y border-line py-6"><div><span className="eyebrow">Reviews collected</span><strong className="mt-2 block font-mono text-4xl font-medium tabular">{integer(scan.review_count)}</strong></div><div><span className="eyebrow">Collection</span><strong className="mt-3 block text-sm font-semibold capitalize">{scan.collection_status ?? 'Unknown'}</strong></div></div>{active && <section className="mt-7" aria-label="Analysis progress"><div className="flex items-baseline justify-between gap-4"><span className="text-sm font-semibold">{progress?.message || 'Preparing analysis'}</span><span className="font-mono text-sm tabular text-orange">{percent}%</span></div><div className="mt-3 h-2 overflow-hidden bg-line" role="progressbar" aria-label="Analysis progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent}><div className="h-full bg-orange transition-[width] duration-500" style={{ width: `${percent}%` }} /></div>{hasReviewCount && <p className="mt-2 font-mono text-xs text-muted">{progress.completed} / {progress.total} reviews</p>}</section>}<p className="mt-7 leading-relaxed text-muted">{failed ? scan.error || 'The backend did not provide an error message.' : scan.analysis_status === 'not_started' ? 'The review sample is collected. Start analysis to process sentiment, keywords, issues and actionable insights.' : 'Processing sentiment, keywords, issues and actionable insights. This page updates automatically.'}</p>{(failed || scan.analysis_status === 'not_started') && <button className="thin-button mt-6" onClick={onRetry} disabled={retrying}>{failed ? 'Retry analysis' : 'Start analysis'} <ArrowRight size={15} /></button>}{retryError && <p role="alert" className="mt-3 text-sm text-danger">{retryError}</p>}</div>{active && <div className="activity-line" />}</div>
}
...
function Workspace() {
  ...
  return <Shell><main className="mx-auto max-w-[1480px] px-5 pb-20 pt-8 md:px-10"><Link to="/" className="inline-flex items-center gap-2 text-sm text-muted hover:text-rust"><ArrowLeft size={15} /> New analysis</Link>
    {status.isLoading ? <div className="mt-20 flex items-center gap-3 text-muted"><LoaderCircle size={18} className="animate-spin" /> Loading scan…</div> : status.isError || !scan ? <div className="panel mt-12 p-8"><CircleAlert className="text-danger" /><h2 className="mt-4 font-serif text-3xl">Scan unavailable</h2><p className="mt-2 text-muted">{(status.error as Error)?.message ?? 'The scan could not be found.'}</p></div> : <>
      <div className="mt-9 flex flex-wrap items-end justify-between gap-6 border-b border-ink pb-8"><div><p className="eyebrow">APP {scan.app_id} / SCAN {scan.scan_id}</p><h1 className="mt-3 font-serif text-[clamp(38px,4.6vw,64px)] leading-none tracking-[-0.035em]">{scan.app_name || `App ${scan.app_id}`}</h1><p className="mt-5 text-sm text-muted"><span className="font-mono text-ink">{integer(scan.review_count)}</span> reviews <span className="mx-2 text-line">·</span> {scope}</p></div><div className="flex flex-wrap items-center gap-3"><span className={`mr-2 font-mono text-[11px] font-medium uppercase tracking-[0.14em] ${completed ? 'text-good' : scan.analysis_status === 'failed' ? 'text-danger' : 'text-orange'}`}><span className="mr-2 inline-block h-2 w-2 rounded-full bg-current" />{scan.analysis_status.replaceAll('_', ' ')}</span><a href={downloadReviewsUrl(scanId)} className="thin-button"><ArrowDownToLine size={16} /> Download raw reviews</a>{completed && <><a href={downloadReportUrl(scanId)} className="thin-button"><ArrowDownToLine size={16} /> Download full report JSON</a><a href={downloadPdfReportUrl(scanId)} className="thin-button"><ArrowDownToLine size={16} /> Download report PDF</a></>}</div></div>
