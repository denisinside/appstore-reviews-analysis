...
PDF report downloads render the React report route with Playwright and Chromium. Install the browser once after installing Python dependencies with `python -m playwright install chromium`. The API uses `FRONTEND_URL` (default `http://localhost:5173`) as the frontend origin when printing reports.
